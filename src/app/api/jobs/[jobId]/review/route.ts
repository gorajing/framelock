...
import { reviewRealJob } from "@/lib/review/real-job-review.server";
...
export async function POST(
  ...
  try {
    if ([...new URL(request.url).searchParams].length > 0) {
      throw invalidRequest("The review action accepts no query parameters");
    }
    if ((await request.text()).trim() !== "") {
      throw invalidRequest("The review action accepts no request body");
    }
    const { jobId: rawJobId } = await context.params;
    const jobId = jobIdSchema.parse(rawJobId);
    return NextResponse.json(await reviewRealJob(jobId), noStore(200));
  } catch (error) {
