...
import { RealJobReviewError } from "@/lib/review/real-job-review";
import { resumeRealJobVerification } from "@/lib/review/real-job-review.server";
...
function errorResponse(error: unknown) {
  ...
  if (error instanceof RealJobReviewError) {
    const status = ["ACTIVE_JOB_MISMATCH", "INVALID_JOB_STATE"].includes(
      error.code,
    )
      ? 409
      : 500;
  ...
  try {
    if ([...new URL(request.url).searchParams].length > 0) {
      throw invalidRequest(
        "The verification resume action accepts no query parameters",
      );
    }
    if ((await request.text()).trim() !== "") {
      throw invalidRequest("The verification resume action accepts no body");
    }
    const { jobId: rawJobId } = await context.params;
    const jobId = jobIdSchema.parse(rawJobId);
    return NextResponse.json(
      await resumeRealJobVerification(jobId),
      noStore(200),
    );
  } catch (error) {
...
function invalidRequest(message: string): z.ZodError {
